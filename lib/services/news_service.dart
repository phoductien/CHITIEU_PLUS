...
class NewsService {
  static const List<Map<String, String>> _sources = [
    {'url': 'https://vneconomy.vn/tai-chinh.htm', 'name': 'VnEconomy'},
    {'url': 'https://vnbusiness.vn/tai-chinh', 'name': 'VnBusiness'},
    {'url': 'https://vnbusiness.vn/rss/tieu-dung.rss', 'name': 'VnBusiness'},
    {'url': 'https://cafef.vn/tai-chinh-ngan-hang.rss', 'name': 'CafeF'},
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          for (var item in rssFeed.items ?? []) {
            ...
            // Filter by keywords
            bool matches = _keywords.any(
              (kw) =>
                  title.toLowerCase().contains(kw) ||
                  description.toLowerCase().contains(kw),
            );

            if (matches || allArticles.length < 5) {
              // Relax filtering if we don't have enough
              allArticles.add(
                NewsArticle(
                  title: title,
                  link: item.link ?? '',
                  ...
                  imageUrl:
                      _extractImageUrl(item.description ?? '') ??
                      _extractImageUrlFromContent(item.content?.value ?? ''),
                  source: source['name']!,
                ),
              );
            }
  ...
  List<NewsArticle> _getFallbackTips() {
    return [
      NewsArticle(
        ...
        link:
            'https://vneconomy.vn/6-quy-tac-vang-giup-ban-tiet-kiem-tien-hieu-qua.htm',
        pubDate: DateTime.now().toString(),
        description:
            'Quản lý tài chính cá nhân là một kỹ năng quan trọng giúp bạn đạt được các mục tiêu trong cuộc sống...',
        source: 'Tư vấn',
      ),
      NewsArticle(
        title: 'Cách lập kế hoạch chi tiêu gia đình trong 1 tháng',
        link:
            'https://vnbusiness.vn/tu-van/cach-lap-ke-hoach-chi-tieu-gia-dinh-trong-1-thang-1082522.html',
        pubDate: DateTime.now().toString(),
        description:
            'Việc lập kế hoạch chi tiêu giúp bạn kiểm soát dòng tiền và tránh những khoản chi không cần thiết...',
        source: 'Tư vấn',
      ),
      NewsArticle(
        ...
        pubDate: DateTime.now().toString(),
        description:
            'Những thói quen nhỏ hàng ngày có thể giúp bạn giảm đáng kể hóa đơn tiền điện nước mỗi tháng...',
        source: 'Mẹo vặt',
      ),
    ];
  }
